...
//			Lister les salles utilisées par une filière
			
			System.out.println("################## Query 11");

			TypedQuery<String> query11 = em.createQuery("select s.nom from Stagiaire s where s.filiere.referent.nom = :nom",
					String.class);

			query11.setParameter("nom", "SULTAN");

			for (String nom : query11.getResultList()) {
				System.out.println(nom);
			}
