...
public class TestJPQL {
	...
	public static void main(String[] args) {
		...
		try {
			...
			TypedQuery<Salle> query11 = em.createQuery("select distinct s from Salle s join s.ues u where u.filiere.promotion = :promo",
					Salle.class);

			query11.setParameter("promo", "COVID");

			for (Salle salle : query11.getResultList()) {
			...
			TypedQuery<Salle> query12 = em.createQuery("select distinct s from Salle s where s.adr.ville = :nom",
					Salle.class);

			query12.setParameter("nom", "Mérignac");

			for (Salle salle : query12.getResultList()) {
			...
			TypedQuery<Formateur> query13 = em.createQuery("select f from Formateur f where f.email = :mail",
					Formateur.class);

			query13.setParameter("mail", "e.sultan@ajc-ingenierie.fr");
			...
			TypedQuery<Formateur> query14 = em.createQuery("select f from Filiere f where f.ues.salle.adr.ville = :ville",
					Formateur.class);

			query14.setParameter("ville", "Mérignac");
